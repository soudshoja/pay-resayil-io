...
@section('title', __('messages.nav.sales_persons'))
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-white">{{ __('messages.nav.sales_persons') }}</h1>
            <p class="text-gray-400">{{ __('messages.nav.sales_persons') }} ({{ $salesPersons->total() }})</p>
        </div>
        <a href="{{ route('client.sales-persons.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:from-purple-600 hover:to-pink-600 transition-all">
            ...
            <table class="w-full">
                <thead class="bg-dark-800">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('messages.nav.agents') }}</th>
                ...
                <tbody class="divide-y divide-gray-800">
                    @forelse($salesPersons as $user)
                        <tr class="hover:bg-dark-800/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-gray-300">{{ $user->managed_agents_count }}</td>
                            <td class="px-6 py-4">
                                @if($user->is_active)
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('client.sales-persons.edit', $user) }}" class="p-2 text-gray-400 hover:text-blue-400 transition-colors" title="{{ __('messages.common.edit') }}">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <form action="{{ route('client.sales-persons.destroy', $user) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('messages.common.confirm_delete') }}')">
                                        @csrf
                                        @method('DELETE')
